...
const Header = () => {
  ...
  const mainNavRight = [
    ...
    { display: "Đăng nhập", path: "/login", icon: "bx bxs-user" },
  ];
  let { pathname } = useLocation();
  if (
    pathname === "/catalog/gift" ||
    pathname === "/catalog/food" ||
    pathname === "/catalog/confectionery"
  ) {
    pathname = "/catalog";
  }
            ...
            {mainNavLeft.map((item, index) => (
              ...
              >
                <Link to={item.path}>
                  ...
                  <span onClick={toggleNav}>{item.display}</span>
                </Link>
              </div>
            ))}
          </div>
